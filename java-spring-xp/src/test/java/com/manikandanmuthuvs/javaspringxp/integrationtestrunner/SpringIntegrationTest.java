...
@SpringBootTest(classes = JavaSpringXpApplication.class, webEnvironment = WebEnvironment.DEFINED_PORT)
...
public class SpringIntegrationTest {
    ...
    public void executePost() throws IOException {
        ...
        restTemplate.setErrorHandler(errorHandler);
        latestResponse = restTemplate.execute("http://localhost:8081/baeldung", HttpMethod.POST, requestCallback,
                response -> {
                    if (errorHandler.hadError) {
                        return (errorHandler.getResults());
                    } else {
